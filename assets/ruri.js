...
const dialog=$('#search-dialog'),input=$('#search-input'),results=$('#search-results'),status=$('#search-status');let indexPromise;
...
let searchVersion=0,timer;input?.addEventListener('input',()=>{clearTimeout(timer);const version=++searchVersion;timer=setTimeout(async()=>{const query=input.value.trim().toLocaleLowerCase();results.replaceChildren();if(!query){status.textContent='';return}status.textContent=tr('searching');try{indexPromise??=fetch(dialog.dataset.index).then(r=>{if(!r.ok)throw Error();return r.json()}).catch(e=>{indexPromise=null;throw e});const entries=await indexPromise;if(version!==searchVersion)return;const words=query.split(/\s+/);const found=entries.filter(x=>words.every(w=>(x.title+' '+x.text+' '+(x.tags||[]).join(' ')).toLocaleLowerCase().includes(w)));status.textContent=found.length+tr('results');found.slice(0,50).forEach(x=>{const a=document.createElement('a'),s=document.createElement('small');a.href=x.url;a.textContent=x.title;s.textContent=x.text.slice(0,110)+'…';a.append(s);results.append(a)})}catch{status.textContent=tr('retry')}},180)});

initTabs();initComments();document.querySelectorAll('.toc ul ul').forEach(ul=>{const li=ul.parentElement;if(!li||li.querySelector(':scope > details'))return;const link=li.querySelector(':scope > a'),d=document.createElement('details'),summary=document.createElement('summary');const depth=li.closest('ul')?.parentElement?.closest('ul')?3:2;d.open=false;if(link){summary.innerHTML=link.outerHTML;link.remove()}d.append(summary,ul);li.append(d)});loadMusic();const toc=document.querySelector('.toc');if(toc){const links=[...toc.querySelectorAll('a[href^=\"#\"]')],heads=[...document.querySelectorAll('.prose h2,.prose h3,.prose h4,.prose h5,.prose h6')];const reveal=id=>{toc.querySelectorAll('details').forEach(d=>d.open=false);const a=links.find(x=>x.getAttribute('href')==='#'+id);if(a){let d=a.closest('details');while(d){d.open=true;d=d.parentElement.closest('details')}}};const observer=new IntersectionObserver(es=>{const hit=es.filter(e=>e.isIntersecting).sort((a,b)=>a.boundingClientRect.top-b.boundingClientRect.top)[0];if(hit)reveal(hit.target.id)},{rootMargin:'-18% 0px -68% 0px',threshold:0});tocObserver=observer;heads.forEach(h=>observer.observe(h))}document.querySelectorAll('.copy-link').forEach(b=>b.addEventListener('click',async()=>{try{await navigator.clipboard.writeText(b.dataset.url);b.textContent=tr('copied')}catch{b.textContent=b.dataset.url}}));
}
